  ...
  const deleteButton = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          ` https://crafty-canvas-server-em5iysxl2-arittros-projects.vercel.app/arts/${id}`,
          {
            method: "DELETE",
          }
        )
      ...
      <div>
        {items.length > 0 ? (
          <h1 className="font-bold text-3xl py-5 text-center">
            Your added arts and craft is : {items.length}
          </h1>
        ) : (
          <div className="text-center">
            ...
            <Link
              to={"/AddArts"}
              className="text-green-600 flex items-center gap-5"
            >
              <FaArrowLeftLong className="my-5 text-3xl text-green-600"></FaArrowLeftLong>
              Add Your Item
            </Link>
          </div>
        )}
          ...
          <div className="pb-10 text-center">
            <details className="dropdown">
              ...
              <ul className="p-2 shadow menu dropdown-content z-[1] bg-base-100 rounded-box w-52">
                <li onClick={() => hanldeYes("Yes")}>
                  <a>Yes</a>
                </li>
                <li onClick={() => handleNo("No")}>
                  <a>No</a>
                </li>
              </ul>
            </details>
            ...
            <div className="bg-slate-50 text-slate-900 p-5 rounded-xl shadow-2xl border hover:border-green-500 duration-300 transition ">
              <div className="relative">
                ...
                <div className="absolute top-2 left-2">
                  <p className="font-bold bg-green-600 text-white px-4 py-2 rounded-full">
                    {art.stockStatus}
                  </p>
                </div>
              </div>
              <div className="space-y-4">
                ...
                <div className="flex space-y-3 flex-col justify-between">
                  <p className="font-bold">
                    Customaization : {art.customization}
                  </p>
                  <p className="flex items-center gap-2">
                    Proccesing Time : {art.processing_time}
                  </p>
                </div>
              </div>
              <div className="flex items-center justify-between gap-6 py-5">
                <Link
                  to={`/updateArts/${art._id}`}
                  className="font-bold bg-green-600 text-white px-6 py-3 rounded hover:bg-green-700"
                >
                  Update
                </Link>
                <Link
                  onClick={() => deleteButton(art._id)}
                  className="font-bold bg-green-600 text-white px-6 py-3 rounded hover:bg-green-700"
                >
                  Delete
                </Link>
